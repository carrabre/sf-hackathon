...
kuzco_api_key = settings.KUZCO_API_KEY

@api_view(['POST'])
def handle_query(request):
    query = request.data.get('query')
    model_name = request.data.get('model')
    use_server_api = request.data.get('use_server_api', False)
    logger.info(f"Received query: {query} for model: {model_name}, use_server_api: {use_server_api}")

    try:
        if use_server_api:
            # Use server API for all models
            url = f"https://{SERVER_IP}:{SERVER_PORT}/api/query/"
            headers = {
                "Authorization": f"Bearer {kuzco_api_key}",
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Received response from server API for {model_name}")
            else:
                error_message = response.json().get('error', {}).get('message', '')
                logger.error(f"Error from server API: {response.status_code}, {error_message}")
                reply = f"Error: {response.status_code}, {error_message}"
        else:
            if model_name in ['gpt-4', 'gpt-4-turbo-2024-04-09', 'gpt-4o', 'o1-preview']:
                # OpenAI models
...
def stream_query(request):
    if request.method == 'GET':
        ...
        use_server_api = request.GET.get('use_server_api', 'false').lower() == 'true'
        ...
        if request.content_type == 'application/json':
            ...
            use_server_api = data.get('use_server_api', False)
        else:
            query = request.POST.get('query')
            model_name = request.POST.get('model')
            context = json.loads(request.POST.get('context', '[]'))
            use_server_api = request.POST.get('use_server_api', 'false').lower() == 'true'
    else:
        return HttpResponse("Method not allowed", status=405)

    logger.info(f"Streaming query: {query} for model: {model_name} with context: {context}, use_server_api: {use_server_api}")

    def event_stream():
        try:
            response_generator = get_model_response(model_name, query, context, use_server_api)
...
def get_model_response(model_name, query, context, use_server_api):
        ...
        try:
            with requests.post(url, headers=headers, json=payload, stream=True, timeout=90) as response:
                if response.status_code == 200:
                    for line in response.iter_lines():
                        if line:
                            ...
                            if line.startswith('data: '):
                                if line.strip() == 'data: END':
                                    logger.info("Received END marker from server API")
                                    break
                                try:
                    ...
                    error_message = f"Error from server API: {response.status_code}, {response.text}"
                    logger.error(error_message)
                    yield error_message
        except requests.exceptions.RequestException as e:
            logger.error(f"Request to server API failed: {str(e)}")
            yield f"Error: Connection to server API failed - {str(e)}"
    else:
        if model_name in ['gpt-4', 'gpt-4-turbo-2024-04-09', 'gpt-4o', 'o1-preview']:
                    ...
                    if response.status_code == 200:
                        for line in response.iter_lines():
                            if line:
                                ...
                                if line.startswith('data: '):
                                    if line.strip() == 'data: END':
                                        logger.info("Received END marker from LLaMA API")
                                        break
                                    try:
                                        data = json.loads(line[6:])
                                        if data['choices'][0]['delta'].get('content'):
